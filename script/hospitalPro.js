...
function validateForm(event) {
  ...
  var contactNumber = document.forms["hospitalForm"]["contact"].value;
  if (!/^\d{10}$/.test(contactNumber)) {
      alert("Please enter a valid 10-digit contact number.");
      return false;
  }

  // Validation for Helpline Number
  var helplineNumber = document.forms["hospitalForm"]["helplineNo"].value;
  if (!/^\d{10}$/.test(helplineNumber)) {
